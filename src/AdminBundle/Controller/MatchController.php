...
class MatchController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $date = \DateTime::createFromFormat( 'Y-m-d H:i:s', $em->getRepository( 'MainBundle:Edition' )->getDate( 'inscription', 'end' ) );
    ...
    public function ajaxUpdateFeuilleAction( Request $request ) {
        if( $request->isXmlHttpRequest() ) {
            try {
                ...
                $feuille->setWinner( $forfait ? ( $teamAttackData[ 'forfait' ] ? $match->getDefense() : $match->getAttack() ) : ( $teamAttackData[ 'morts' ] == $this->em->getRepository( 'AdminBundle:Config' )->getOneBy( array( 'name' => 'nb_players_team' ) ? $match->getDefense() : $match->getAttack() ) );
